...
(def ^:private family-buckets
  "Pairs of [exclusive-upper-hue-bound family-keyword] on the HSLuv hue
   wheel, scanned in order. HSLuv hues sit at different angles than HSL
   hues (pure red is 12, yellow 86, green 128, blue 266), so these bounds
   are not interchangeable with HSL ones."
...
(defn metadata
  ...
  ([rgbas weights]
   (let [n      (count rgbas)
         ...
         family     (if (< saturation 0.14) :neutral (hue->family mean-hue))
