(ns clj-colors.main
  ...
  (:require [clojure.edn :as edn]
            ...
            [clj-colors.weights.base :as weights])
...
(def system-resources
  ["resources"
   "resources/extensions/associations"
   "resources/extensions/palettes"
   "resources/extensions/colors"])
...
(defn palette-distribution
  [hexes weights]
  (zipmap (map str/lower-case hexes) weights))

(defn- enrich
  [k palette]
  (let [rgbas       (normalize-colors palette)
        hexes       (or (:hex palette) (mapv color/rgba->hex rgbas))
        explicit-ws (normalize-weights (:weights palette) (count rgbas))
        weights     (or explicit-ws
                        (vec (repeat (count rgbas) (/ 1.0 (count rgbas)))))
        computed    (meta/metadata rgbas weights)
        category    (when (namespace k) (keyword (namespace k)))
        dist        (palette-distribution hexes weights)
        oklabs      (color-tags/palette-oklabs hexes)
        attrs       (associations/palette-attributes hexes weights dist oklabs)
        base        (merge (dissoc computed :tags)
                           {:hex               (vec hexes)
                            :rgba              rgbas
                            :count             (count rgbas)
                            :name              (keyword (name k))
    ...
    (cond-> base
      explicit-ws (assoc :weights explicit-ws))))
...
(defn load-palettes
  "Read and enrich palettes, then sync the enriched entries back
   to extension files so on-disk files match the in-memory form.
   With no argument, reads the default-resource from the classpath.
   With a path string or File, reads that file directly. Palette
   extensions from resources/extensions/palettes/ are merged on
   top of the base resource before enrichment, so attributes are
   computed against the full extended set.

   Returns the enriched registry map for the caller to install
   (the defonce wraps it in an atom on first call; reload-all!
   resets the existing atom on subsequent calls)."
  ([] (load-palettes (io/resource default-resource)))
  ([source]
   (let [source      (or source default-resource)
         raw         (edn/read-string (slurp source))
         ext-by-file (extensions/load-palette-extensions)
         all-raw     (merge raw (extensions/merge-files ext-by-file))
         enriched    (enrich-registry all-raw)]
     (extensions/sync-back! ext-by-file enriched)
     enriched)))
...
; Random
(defn random-palette
  "A random [key palette] entry from the registry."
  []
  (when (seq @registry) (rand-nth (vec @registry))))
...
(defn- palette->lines
  "Render one palette as materialized EDN lines."
  [k palette]
  (let [present (filter #(contains? palette %) field-order)
        body    (map (fn [field]
                       (str "   " field " " (field-str field (get palette field))))
                     present)
        body    (cons (str "  {" (subs (first body) 3)) (rest body))
        body    (concat (butlast body) [(str (last body) "}")])]
    (concat [(str " " (pr-str k))] body)))
...
(defn refresh-band-weights!
  []
  (weights/invalidate!)
  (weights/invalidate-palette!)
  (weights/weights)
  (weights/palette-weights (vals @registry)))

(defn refresh-all-attributes!
  "Recompute :attributes for every palette in the registry..."
  []
  (run! compression/ensure-all-unpacked! system-resources)
  (associations/refresh!)
  (color-tags/refresh-signatures! @associations/data)
  (cc/refresh! @registry @associations/data)

  (dotimes [_ 2]
    (refresh-band-weights!)

    (let [palettes @registry
          total    (count palettes)]
      (doseq [[idx [k p]] (map-indexed vector palettes)]
        (println (format "Refreshing %d of %d (%s)"
                         (inc idx)
                         total
                         k))
        ...
        (let [hexes       (:hex p)
              ...
              dist        (palette-distribution hexes ws)
              oklabs      (color-tags/palette-oklabs hexes)
              palette-cat (some-> k namespace keyword)
              attrs       (associations/palette-attributes
                           hexes ws dist oklabs
                           {:palette-category palette-cat})]
          (swap! registry assoc-in [k :attributes] attrs))))))